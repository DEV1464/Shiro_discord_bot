...
import wikipedia	from bs4 import BeautifulSoup
...
async def on_ready():
 ...
 await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.streaming, name="ENTER THE STATUS NAME AS YOU WANT" ))
...
async def on_message(message):
    ...
    if message.content == 'FOLLOW THE RULES':
        ...
        notverified=discord.utils.get(message.guild.roles, name="❎™️")
...
async def joke(ctx):
 ...
 async with aiohttp.ClientSession() as session:
          ...
          embed = discord.Embed(title="lel,😂🤣", color=ctx.guild.me.top_role.color)
